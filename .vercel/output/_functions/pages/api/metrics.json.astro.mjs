...
const PROMETHEUS_URL = "http://72.61.162.156:9090"?.trim();
const PANEL_BASE = "https://panel.gmhost.es/server"?.trim()?.replace(/\/$/, "");
const parseValue = (result) => {
  if (!result) return 0;
  const raw = result.value?.[1];
  const value = Number(raw);
  return Number.isFinite(value) ? value : 0;
};
const toNumberMap = (results, keyOf) => {
  const map = {};
...
const runQuery = async (query, atUnixSeconds) => {
...
const parseRequestedAt = (raw) => {
  ...
  return {
    atUnixSeconds: Math.floor(date.getTime() / 1e3),
...
const GET = async ({ request }) => {
  try {
    ...
    for (const info of nodeInfo) {
      ...
      const cpuFreeCores2 = Math.max(0, cpuTotalCores - cpuUsedCores);
      ...
      const memoryFreeBytes2 = Math.max(0, memoryTotalBytes - memoryUsedBytes);
      ...
      const hostMemoryAvail = hostMemAvailByNodeExporter[nodeExporterInstance] ?? memoryFreeBytes2;
      ...
      const diskFreeBytes2 = diskAvailByNodeExporter[nodeExporterInstance] ?? 0;
      const diskUsedBytes = Math.max(0, diskTotalBytes - diskFreeBytes2);
      ...
      const volumesTotalBytes = hasVolumesMetrics ? Number.isFinite(volumesTotalRaw) ? volumesTotalRaw : diskTotalBytes : null;
      const volumesUsedSanitized = hasVolumesMetrics ? Math.max(0, volumesUsedRaw) : null;
      const volumesUsedBytes = hasVolumesMetrics && volumesUsedSanitized !== null && volumesTotalBytes !== null ? Math.min(volumesUsedSanitized, volumesTotalBytes) : null;
      const volumesFreeBytes = hasVolumesMetrics && volumesTotalBytes !== null ? Number.isFinite(volumesAvailRaw) ? Math.max(0, volumesAvailRaw) : volumesUsedBytes !== null ? Math.max(0, volumesTotalBytes - volumesUsedBytes) : null : null;
      const volumesUsagePercent = hasVolumesMetrics && volumesTotalBytes && volumesUsedBytes !== null ? clamp(volumesUsedBytes / volumesTotalBytes * 100) : null;
    ...
    const containers = containerMemById.map((item) => {
